...
import { PackLibraryProps, PackLibraryState } from '../../@types/pack';
import Modal from './Modal';
import { DeviceSection } from './Device/DeviceSection';
import { LibrarySection } from './Library/LibrarySection';
import { usePackLibraryActions } from './Pack/usePackLibraryActions';

export const PackLibrary: React.FC<PackLibraryProps> = () => {
    const { t } = useTranslation();
    const dispatch = useAppDispatch();

    const { device, library, settings } = useAppSelector(state => state);


    const [state, setState] = React.useState<PackLibraryState>({
        searchTerm: null,
        ageMinFilter: 0,
        ageMaxFilter: 12,
    ...
    const {
        ...
        handleCreateNew,
        handleOpenSample
    } = usePackLibraryActions(state, setState);

    return (
        <div className="pack-library">
            <Modal
                isOpen={state.showRemoveFromDeviceConfirmDialog}
                title={t('dialogs.library.removeFromDevice.title')}
                content={t('dialogs.library.removeFromDevice.content')}
                onConfirm={() => handleRemoveFromDevice(state.removingFromDevice)}
                onCancel={() => setState(prev => ({ ...prev, showRemoveFromDeviceConfirmDialog: false }))}
            />

            {device.metadata && (
                <DeviceSection
                    device={device}
                    onDrop={handleDropIntoDevice}
                    onRemove={(uuid) => setState(prev => ({
                        ...prev,
                        removingFromDevice: uuid,
                        showRemoveFromDeviceConfirmDialog: true
                    }))}
...
// import { actionAddFromLibrary, actionRemoveFromDevice, actionReorderOnDevice } from '../actions/device.actions';
// import { actionLoadPackInEditor, actionCreatePackInEditor, actionLoadSampleInEditor } from '../actions/editor.actions';
// import { actionDownloadFromLibrary, actionConvertInLibrary, actionRemoveFromLibrary, actionUploadToLibrary } from '../actions/library.actions';
// /** New import */
...
//         const packData = event.dataTransfer.getData("local-library-pack");
//         if (!packData) {
...
//         // Get the latest pack
//         const latestPack = data.packs[0];
//         console.log('latest pack: %o', latestPack);
//         // Get the latest device-compatible pack
//         const compatiblePack = data.packs.find(p => p.format === this.state.device.metadata.driver);
...
//                 const file = this.getDroppedFile(event);
...
//                             const rect = event.target.closest('.device-dropzone').getBoundingClientRect();
...
//                                     }}
//                                     onDragEnter={event => {
//                                         const data = this.state.reordering;
//                                         if (data && data.uuid !== pack.uuid) {
//                                             const reordered = this.state.device.packs;
...
//                                             const uuids = this.state.device.packs.map(p => p.uuid);
